		...
		std::vector<int>& _c_elim)
{
  ...
  for(unsigned int i=0; i<gmm::mat_nrows(_constraints); ++i)
  {
    ...
    auto row_it = gmm::vect_const_begin(row);
    auto row_end = gmm::vect_const_end(row);
  ...
  while(!queue.empty())
  {
    ...
    for(; row_it != row_end; ++row_it)
    {
      ...
      if(  cur_j != n_vars - 1 )
      {
        ...
        if( !roundmap[ cur_j ])
        {
          if (std::abs((double)(*row_it)) > max_elim_val)
          {
              elim_j = cur_j;
              max_elim_val = std::abs((double)(*row_it));
          }
          //break;
        }
        else
        {
          double cur_row_val(std::abs((double)(*row_it)));
          // gcd
          // if the coefficient of an integer variable is not an integer, then
          // the variable most problably will not be (expect if all coeffs are the same, e.g. 0.5)
          if( (double(int(cur_row_val))- cur_row_val) != 0.0)
                       ...
                       int& _n_ints)
{
  ...
  auto row_it    = gmm::vect_const_begin( gmm::mat_const_row( _constraints, _row_i));
  auto row_end   = gmm::vect_const_end( gmm::mat_const_row( _constraints, _row_i));
